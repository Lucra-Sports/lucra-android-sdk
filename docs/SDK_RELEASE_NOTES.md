## 7.0.1
* `Tournament` now carries `visibilityLevel` and `totalParticipants`, plus an `isPrivate` convenience property. Purely additive. See [3.3_tournaments_headless.md](3.3_tournaments_headless.md).
* Added a `LucraClient.queryRecommendedTournaments(...)` overload that takes `includePrivateViewableTournaments`. The existing overload is unchanged and still excludes private tournaments. When `true`, the response also includes private tournaments that are viewable without a join code — a code is still required to join them.

## 7.0.0
* Added handshake auth. Silent sign-in backed by your own session, replacing phone auth for signed-in users of your app. Purely additive and non-breaking. New `LucraClient` surface: `registerHandshakeAuthTokenProvider(provider: (suspend () -> String)?)`, `signInWithHandshakeAuth(onResult: (SignInWithHandshakeAuthResult) -> Unit)`, `isHandshakeAuthInFlight: StateFlow<Boolean>`, and `handshakeAuthError: Flow<HandshakeAuthError?>`, with new types `SignInWithHandshakeAuthResult` and `HandshakeAuthError`.
* Handshake auth now captures Terms of Service acceptance for brand new users. A sign-in for a user who has no Lucra account yet fails with the new `HandshakeAuthError.TosNotAccepted` — present the new `LucraFlow.HandshakeTOS` flow, which captures the agreement and resubmits the sign-in itself. Unlike every other handshake failure, this one is not a cue to fall back to phone auth. When Lucra is presenting the flow, that step is shown inline automatically and continues into the flow you asked for; users who already have a Lucra account are unaffected.
* `signInWithHandshakeAuth` now waits for any stored session to finish restoring before it exchanges, and reports the user who is already signed in rather than minting a second session. It is safe to call unconditionally at app start. To switch users, call `logout` first.
* Lucra components now run handshake auth on a signed-out tap instead of presenting Lucra's login screen inside your layout, falling back to it only if the exchange fails. Applies to every component; integrations with no handshake provider registered are unaffected.
* `handshakeAuthError` now clears when a new sign-in attempt starts, not only when one succeeds. A retry no longer runs with the previous attempt's error still standing.
* Added `registerHandshakeAuthTokenProvider(bypassTosAgreement: Boolean, provider: (suspend () -> String)?)` for integrators whose own onboarding already captures agreement to Lucra's terms. The bypass applies only to tenants Lucra has permitted; otherwise it is ignored, the Terms of Service step applies as usual, and a log records the request. Talk to Lucra before setting it.
* [Breaking Change] Improved phone-auth errors with `MessagingDisabled`, `SmsNotDelivered`, and `TooManyAttempts`; invalid codes no longer return `Unknown`. The new variants are added to the public sealed `PhoneAuthError`, so exhaustive `when` expressions over it must be updated. See [Headless Interactions](1.2.9_headless_interactions.md#phone-authentication).
* [Breaking Change] The SDK now depends on `io.sentry:sentry` **8.54.0** for its own internal diagnostics (it never calls `Sentry.init` and sends nothing to your Sentry project). If your app also uses Sentry, every `io.sentry:*` artifact must resolve to 8.54.0 or newer; an older version can crash at launch with `Sentry SDK has detected a mix of versions` or fail with `AbstractMethodError`. See [Project Setup](1.0.0_project_setup.md#sentry-version-alignment).
* Added handshake auth. Silent sign-in backed by your own session, replacing phone auth for signed-in users of your app. Purely additive and non-breaking.
* Documented that the host Activity must leave `android:taskAffinity` unset (default application affinity). See [Project Setup](1.0.0_project_setup.md#task-affinity).
* [Breaking Change] : `ClientTheme` now takes separate `lightColorStyle` and `darkColorStyle` parameters, and its single `colorStyle` property no longer exists. The deprecated `ClientTheme(colorStyle, fontFamily)` constructor now maps `colorStyle` to `darkColorStyle` only.
* Added light mode support to the SDK UI. Supplying only `darkColorStyle` (or using the deprecated constructor) keeps the SDK locked to dark, supplying only `lightColorStyle` locks it to light, and supplying both makes the SDK follow the system appearance. See [Theming/Appearance](1.2.1_theming.md) and the `uiMode` host-Activity requirement in [Project Setup](1.0.0_project_setup.md#appearance-lightdark-configuration-changes).
* Added an optional `themeMode` to `ClientTheme` so you can force the SDK's appearance instead of letting it follow the device: `ThemeMode.LIGHT` always renders light, `ThemeMode.DARK` always renders dark, and `ThemeMode.AUTO` follows the device's light/dark setting. Omitting it preserves 6.9.0 behavior exactly — the appearance is still derived from which color styles you supply. If a forced mode has no matching `ColorStyle`, the palette you did supply is reused for it (and a warning is logged), so check those colors against the other appearance. `themeMode` is read once, when `LucraClient.initialize` first runs. See [Theming/Appearance](1.2.1_theming.md#theme-mode).
* Fixed the close button on the deposit sheet being invisible when the SDK renders light.
* Updated Sardine MDI SDK from 1.2.67 to **2.0.0**.
* Refreshed the SDK's dark-mode background, surface, and tertiary color tokens to the 2026 Lucra Style Guide values — the blue-tinted neutrals are now a true-neutral gray ramp, and the Lucra-default tertiary is `#FFB364`. Cosmetic only: no API change, and partner-supplied `ColorStyle` values are unaffected.
* Added optional `bannerIconUrl` and `disclaimer` to `LucraTournamentReward` and `LucraCatalogReward`, surfaced by the reward sheet as the reward's banner image and fine print. Both default to `null`, so reads are unaffected.
* Reworked the reward sheet to the unified Minigames design. It now renders as a frosted-glass surface. No API change. See [Reward Sheet](1.2.11_reward_sheet.md#appearance-frosted-glass).
* Added `LucraClient().LucraBackdrop { }`, an optional `@Composable` wrapper around your own content that lets Lucra's sheets blur it directly rather than relying on the platform blur. See [Reward Sheet](1.2.11_reward_sheet.md#getting-the-reliable-blur-with-getlucraflowcomposable).
* `TournamentDetails` (from `LucraClient.retrieveTournamentDetails`) now carries `expiresAt` and `startsAt` as `Date?`, so headless integrations can read a tournament's entry deadline and start time without falling back to the deprecated `retrieveTournament` call. Scores can still be submitted after `expiresAt`, so read `isCompleted` to decide whether a tournament is over. Purely additive and non-breaking. See [3.3_tournaments_headless.md](3.3_tournaments_headless.md).
* Consolidated every SDK screen header onto one pinned `LucraTopBar`, with content scrolling underneath it and frosting through the glass.
